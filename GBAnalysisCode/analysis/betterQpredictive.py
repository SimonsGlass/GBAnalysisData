...
# How much more likely is a particle to rearrange if
# ...its value of X is 1 stddev above the mean vs 1 stddev below
# UPDATE: ...its value of X is above the optimum threshold
...
#axinset = fig2.add_axes([.55, .15, .3, .3])
...
#for Xselection in [56,50,51,52,53,54,55]:#[91,92,93,94,95]:#80,31,32,38,34,35]:
for Xselection in [30,50,80,100]:#30,50,80,100##[86]:#,80,81,82,83,84,85]:#80,31,32,38,34,35]:
    ...
    consideronlytype = 0 # 10 # -1 # considering all atoms would give higher Q's but P_R(mu-sigma) = undefined
    ...
    nbins = 21 # 41 # 410 # TAS 10x more so can optimize thresh for over-under analysis
    ...
    if regenerate == 1:
        ...
        for inum,frame in enumerate(frames):  
            ...
            for delayframe in frame + framespacing*np.arange(1 + delaywindow/framespacing):
                ...
                for iparticle in np.arange(numparticlestoanalyze):
    ...
    #outfilenamebase = 'couplehiresoutfiles/X'+str(Xselection)+'_pR_dset'+dset
    outfilenamebase = 'coupleloresoutfiles/X'+str(Xselection)+'_pR_dset'+dset
    ...
    numhopping = np.sum(numberhopping_vs_bin)
    numconsidered = np.sum(numberineach_bin)
    print(numhopping," is num hopping", numconsidered," is num in bins")
    
    print("Particles above this threshold are this much more likely to hop:")
    # Voronoi Vol [,]): # CS of 100fs-AVE pos [138,139]): # 100fs-AVE PE [153,154]): # SOFT [122,123]):
    #for ibin in np.array([,]): 
    for ibin in np.arange(3):#(135,165,5): 
        #mythresh = np.mean(bin_centers)
        #bin_above_thresh = max(0,(mythresh-binedgemin)/binwidth)
        #bin_above_thresh = min(round(bin_above_thresh), nbins-1)
        bin_above_thresh = ibin
        numabovethresh = np.sum(numberineach_bin[bin_above_thresh:])
        hoppersabovethresh = np.sum(numberhopping_vs_bin[bin_above_thresh:])
        print("Fraction hoppers above thresh:", hoppersabovethresh/np.sum(numberhopping_vs_bin))

        print(str(bin_centers[bin_above_thresh]),
        ((numconsidered-numabovethresh)/numabovethresh*hoppersabovethresh/(numhopping-hoppersabovethresh)) )
    ...
    CDF = np.cumsum(1.0*numberineach_bin/np.sum(numberineach_bin))
    Nconsidered = np.sum(numberineach_bin)

    #if 0: # Identify mean and mean +/- stddev on the CDF
    ...
    #    
    #    print(len(CDF), CDF[0], CDF[len(CDF)-1])
    ...
    #print("smoothing")
    #fractionhopping_vs_bin=smooth(fractionhopping_vs_bin,2)
    axs2.errorbar(CDF, fractionhopping_vs_bin, yerr=1.0/numberineach_bin, label=str(Xselection), color=hotclr[np.mod(Xselection,8)])
    if Xselection == 50:
        axs2.errorbar([0,0.5,1], np.array([1.0,1.0,1.0])*np.sum(numberhopping_vs_bin)/Nconsidered, color='grey')
    ...
    # I thought I might plot the slope as an inset, but is not the right message at all.
    #axinset.plot(0.5*(CDF[0:len(CDF)-1]+CDF[1:len(CDF)]), np.diff(fractionhopping_vs_bin)/fractionhopping_vs_bin[0:len(CDF)-1], label=str(Xselection), color=hotclr[np.mod(Xselection,8)])
    


#axinset.axis([0,1,1e3*-.0001,1e3*.002])

axs2.set_xticks([.1,.2,.3,.4,.5,.6,.7,.8,.9])
axs2.grid(b=True, which='major')
axs2.grid(b=True, which='minor')
#plt.xticks([.1,.2,.3,.4,.5,.6,.7,.8,.9],visible=True)
#plt.yticks(np.array([1e-3,1e-2]),visible=True)
axs2.legend(title='Selection',loc=4,prop={'size':12},bbox_to_anchor=(0.25,0.6))
axs2.set_yscale('log')
#axs2.set_xscale('log')
#axs2.axis([1,5e-3,3e-6,3e-2])
#axs2.axis([5e-3,1,3e-6,3e-2])
#axs2.axis([1,5e-3,0,3e-2])
#axs2.axis([1,0,0,3e-2])
axs2.axis([0,1,1e-5,3e-2])


savelabel='_20171214' # '_20170503'
#axs.set_yscale('log')
